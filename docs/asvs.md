...
| 1.9 Communications encrypted in transit | Met | TLS for all mail protocols; outbound upgrades to verified TLS under MTA-STS/DANE/REQUIRETLS. See V9. |
...
| 9.2.1 Outbound connections use verified TLS where required | Met | Outbound `tls_connect` uses webpki roots + ServerName verification, no `dangerous()`/custom verifier (`src/queue/client.rs`). MTA-STS-enforce / DANE / REQUIRETLS set `require_tls`; a remote without STARTTLS yields a **transient** error (retry, never cleartext). |
