...
| Threat | Control | Evidence |
|---|---|---|
| A vulnerable or yanked crate | `cargo audit --deny warnings` on the main lockfile and on `fuzz/Cargo.lock` (`.github/workflows/reusable-rust-audit.yml:83-108`), weekly and on every lockfile change (`rust-audit.yml:10-27`); `cargo deny` allows crates.io only and an explicit license list (`deny.toml:38-42`); Dependabot covers `/`, `/fuzz` and the signing script's pip requirements (`.github/dependabot.yml:15-43`) | gate: `rust-audit.yml`; no advisory is ignored as of 2026-09-03 |
...
- **Verification that the isolated account is set up as intended.** Since 2026-09-03 the package creates it: `debian/epistle.postinst` adds `glyndor-epistle` with `adduser --system`, gives it `/var/lib/glyndor/epistle` at `0700`, adds the `100000-165535` subuid/subgid ranges, enables linger, runs `podman system migrate` and applies the port floor from `debian/epistle.sysctl`; the sample unit runs under that account (`docs/epistle.service`) instead of a transient one. What is still missing is the other half: the postinst only attempts each step and warns on stderr, so nothing yet refuses to run on a host where the ranges, the linger flag or the floor did not take. `epistle init` is where that check belongs.
...
- **DMARC and TLS-RPT reports are unauthenticated input.** Any host on the internet can drop a forged report into `postmaster@` or `tlsrpt@`; the ingester accepts it on the sender's word (`src/storage/delivery_reports.rs:21`). A forged report cannot move mail, change policy, or read anything, but it can plant a JSONL line under `data_dir/reports/{dmarc,tlsrpt}/{YYYYMMDD}/{org}.jsonl` and influence what the operator reads with `epistle reports` (`src/cli/reports.rs:48`). The line is bounded by the per-recipient rate limits on port 25, the [`MAX_COMPRESSED`] / [`MAX_DECOMPRESSED`] gates in `src/reports/decompress.rs`, the field-length and list caps in `src/reports/bounds.rs`, and the 90-day prune in `src/reports/store.rs`. Inside those bounds the ingester does not verify the sender; the operator reads the JSONL, decides what to do with the information, and the server drives no automatic decision from it. A report whose `org_name` is a path traversal lands inside the day directory as a single segment (`src/reports/bounds.rs:45`), so a forged report cannot escape `data_dir/reports/`.
...
- **No advisories are ignored** (as of 2026-09-03). The `RUSTSEC-2023-0071` ignore that `deny.toml` and `.cargo/audit.toml` carried through the sqlx 0.8 lifecycle is gone with the sqlx 0.9 bump: 0.9 drops `rsa` and `rand 0.8` from the graph, so the warning no longer reaches this lockfile (`fuzz/Cargo.lock` the same), and the gate now reports nothing to ignore.
