...
#[test]
fn queue_list_shows_null_sender_and_empty_spool() {
	use crate::smtp::session::AcceptedMessage;
	use crate::storage::FsSpool;
	let dir = tempfile::tempdir().expect("tempdir");
	// Empty spool: just the total line.
	let mut out = Vec::new();
	assert_eq!(queue::list(dir.path(), &mut out), ExitCode::SUCCESS);
	assert!(String::from_utf8_lossy(&out).contains("0 queued"));

	// A bounce (null reverse-path) renders as `<>`.
	FsSpool::open(dir.path())
		.expect("spool")
		.store(&AcceptedMessage {
		...
		.expect("store");
	// A corrupt spool entry is listed but skipped on load (no crash).
	std::fs::write(
		dir.path()
			.join("spool")
			.join("new")
	...
	.expect("write");
	let mut out = Vec::new();
	queue::list(dir.path(), &mut out);
	assert!(String::from_utf8_lossy(&out).contains("from=<>"));
}

#[test]
fn queue_list_fails_when_spool_unavailable() {
	// A data_dir that is a regular file cannot host the spool directory.
