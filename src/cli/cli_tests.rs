...
fn account_add_creates_and_validates() {
	...
	// Too-short passwords are refused before touching the store.
	assert_eq!(
		accounts::add(
			&config,
			"bob",
			vec!["bob@example.org".into()],
			std::io::Cursor::new("short")
		),
		ExitCode::FAILURE
	);
	// A valid create succeeds and shows up in the listing.
	assert_eq!(
		accounts::add(
			&config,
			"bob",
			vec!["bob@example.org".into()],
			std::io::Cursor::new("a-long-password")
		),
		ExitCode::SUCCESS
	);
	let mut out = Vec::new();
...
#[test]
fn read_line_trims_and_rejects_empty() {
	use std::io::Cursor;
	assert_eq!(read_line(Cursor::new("hello\r\n")).expect("line"), "hello");
	assert!(read_line(Cursor::new("\n")).is_err());
	assert!(read_line(Cursor::new("")).is_err());
}
