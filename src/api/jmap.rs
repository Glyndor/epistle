		...
		.map(|account| {
			(
				...
				json!({
					...
					"isPersonal": true,
					"isReadOnly": false,
					"accountCapabilities": { CORE_CAPABILITY: {} },
				}),
			)
		})
		.collect();
...
pub async fn api(Json(request): Json<Request>) -> Json<Response> {
	let method_responses = request
		.method_calls
		.into_iter()
		.map(|MethodCall(name, args, call_id)| match name.as_str() {
